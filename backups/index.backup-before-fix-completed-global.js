...
const isFutureDate = (value) => {
  ...
  const activeTodayTasks = todayTasks.filter((task) => !isTaskCompleted(task));
  const completedTodayTasks = tasks.filter((task) => isTaskCompletedToday(task));

  const upcomingTasks = tasks
    .filter((task) => !isTaskCompleted(task) && isFutureDate(task.start_time))
    .sort((a, b) => {
      const timeA = normalizeDate(a.start_time)?.getTime() || 0;
      const timeB = normalizeDate(b.start_time)?.getTime() || 0;
      return timeA - timeB;
