...
const isSameDayForHome = (dateA, dateB) => {
...
const getTaskCompletedDateGlobal = (task) => {
  return (
    toSafeDateForHome(task?.completedAt) ||
    toSafeDateForHome(task?.completed_at) ||
    toSafeDateForHome(task?.updatedAt) ||
    toSafeDateForHome(task?.updated_at)
  );
};

const isTaskCompletedGlobal = (task) => {
  return (
...
const isTaskCompletedTodayGlobal = (task) => {
  if (!isTaskCompletedGlobal(task)) return false;

  const completedDate = getTaskCompletedDateGlobal(task);

  if (!completedDate) return false;

  return isSameDayForHome(completedDate, new Date());
};
...
const isFutureDate = (value) => {
  ...
  const activeTodayTasks = todayTasks.filter((task) => !isTaskCompletedGlobal(task));
  const completedTodayTasks = tasks.filter((task) => isTaskCompletedTodayGlobal(task));

  const upcomingTasks = tasks
    .filter((task) => !isTaskCompletedGlobal(task) && isFutureDate(task.start_time))
    .sort((a, b) => {
      const timeA = normalizeDate(a.start_time)?.getTime() || 0;
      const timeB = normalizeDate(b.start_time)?.getTime() || 0;
      return timeA - timeB;
