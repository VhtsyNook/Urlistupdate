...
export default function งานScreen() {
  const router = useRouter();
  const rootNavigationState = useRootNavigationState();

  const [tasks, setงาน] = useState([]);
  ...
  const isUpcoming = (value) => {
    ...
    const todayเริ่ม = new Date();
    todayเริ่ม.setHours(0, 0, 0, 0);

    return date > todayเริ่ม && !isToday(date);
  };
  ...
  const isเสร็จแล้วLateTask = (task) => {
  ...
  const buildQuickRescheduledTimeRange = (task, targetType) => {
    const originalเริ่ม = normalizeDate(task.start_time) || new Date();
    const durationMs = getTaskDurationMs(task);
    const now = new Date();

    let newเริ่ม = new Date();

    if (targetType === "today") {
      newเริ่ม.setHours(
        originalเริ่ม.getHours(),
        originalเริ่ม.getMinutes(),
        0,
        0
      );

      if (newเริ่ม.getTime() <= now.getTime()) {
        newเริ่ม = roundUpToNext15Minutes(addMinutes(now, 15));
      }
    }

    if (targetType === "tomorrow") {
      newเริ่ม.setDate(now.getDate() + 1);
      newเริ่ม.setHours(
        originalเริ่ม.getHours(),
        originalเริ่ม.getMinutes(),
        0,
    ...
    return {
      newเริ่ม,
      newสิ้นสุด: new Date(newเริ่ม.getTime() + durationMs),
    };
  };
  ...
  const isSameปฏิทินDay = (startValue, endValue) => {
  ...
  const isNextปฏิทินDay = (startValue, endValue) => {
    const startDate = normalizeDate(startValue);
    const endDate = normalizeDate(endValue);
  ...
  const formatTaskDateRange = (startValue, endValue) => {
    ...
    if (isSameปฏิทินDay(startDate, endDate)) {
  ...
  const formatTaskTimeRange = (startValue, endValue) => {
    ...
    if (isNextปฏิทินDay(startDate, endDate)) {
  ...
  const getความสำคัญ = (task) => {
    return task.priority || "งานทั่วไป";
  };

  const getความสำคัญStyle = (priority) => {
    const lower = String(priority).toต่ำerCase();
  ...
  const getการทำซ้ำLabel = (task) => {
    if (task?.is_generated_session) return "รอบอ่านหนังสือ";

    if (task?.planning_enabled) {
      return `${task.planned_completed_count || 0}/${task.planned_session_count || 0} sessions`;
    }

    if (!task?.is_recurring) return "ไม่ทำซ้ำ";

    const type = String(task.recurrence_type || "").toต่ำerCase();
    ...
    if (type === "monthly") {
      ...
      if (interval === 1) {
        return `ทุกเดือน on day ${monthDay}`;
      }
  ...
  const getTaskTypeLabel = (task) => {
    ...
    if (isเสร็จแล้วLateTask(task)) return "เสร็จแล้ว Late";
    if (task.is_generated_session) return "รอบอ่านหนังสือ";
    if (task.planning_enabled) return "แผนหลัก";
    if (task.is_recurring) return "Recurring Task";
    if (task.is_completed) return "เสร็จแล้ว";
    return "งานทั่วไป Task";
  };
  ...
  const getDeleteModalชื่องาน = () => {
    if (!deleteTargetTask) return "ลบงาน";

    if (deleteTargetTask.planning_enabled) return "ลบแผนอ่านหนังสือ";
    if (deleteTargetTask.is_generated_session) return "Delete รอบอ่านหนังสือ";
  ...
  const sortงาน = (items) => {
    return [...items].sort((a, b) => {
      if (sortBy === "priority") {
        ...
        const rankA =
          priorityRank[String(a.priority || "normal").toต่ำerCase()] || 4;
        const rankB =
          priorityRank[String(b.priority || "normal").toต่ำerCase()] || 4;
  ...
  const filteredงาน = useMemo(() => {
    const keyword = searchText.trim().toต่ำerCase();

    const result = tasks.filter((task) => {
      const title = String(task.title || "").toต่ำerCase();
      const detail = String(task.detail || "").toต่ำerCase();
  ...
  const taskSections = useMemo(() => {
    ...
    filteredงาน.forEach((task) => {
      if (!task) return;

      if (task.is_completed) {
        if (isเสร็จแล้วLateTask(task)) {
          addOnce(completedLateงาน, task);
          return;
        }

        addOnce(completedงาน, task);
        return;
      }

      if (isOverdueTask(task, referenceNow)) {
        addOnce(overdueงาน, task);
        return;
      }
      ...
      if (task.planning_enabled) {
        addOnce(planningงาน, task);
        return;
      }

      if (task.is_recurring) {
        addOnce(recurringงาน, task);
        return;
      }

      addOnce(normalงาน, task);
    });
    ...
    return [
      {
        key: "overdue-tasks",
        title: "Overdue งาน",
        subtitle: "งาน that passed their end time",
        icon: "alert-circle-outline",
        color: COLORS.danger,
        backgroundColor: "#FEE2E2",
        data: sortงาน(overdueงาน),
      },
      {
        key: "today-sessions",
        title: "งานวันนี้",
      ...
      },
      {
        ...
        color: COLORS.primary,
        backgroundColor: COLORS.primaryLight,
        data: sortงาน(normalงาน),
      },
      {
        key: "recurring-tasks",
        title: "Recurring งาน",
        subtitle: "การทำซ้ำed tasks",
        icon: "repeat-outline",
        color: "#F97316",
        backgroundColor: "#FFEDD5",
        data: sortงาน(recurringงาน),
      },
      {
        key: "completed-late",
        title: "เสร็จแล้ว Late",
        subtitle: "Finished after the planned end time",
        icon: "time-outline",
        color: COLORS.danger,
        backgroundColor: "#FEE2E2",
        data: sortงาน(completedLateงาน),
      },
      {
        key: "completed",
        title: "เสร็จแล้ว",
        ...
        color: COLORS.success,
        backgroundColor: "#DCFCE7",
        data: sortงาน(completedงาน),
      },
    ].filter((section) => section.data.length > 0);
  }, [filteredงาน, sortBy, filter, nowTick]);
  ...
  const handleเสร็จสิ้นTask = async (taskId) => {
    try {
      await updateTaskStatus(taskId, "completed");
    } catch (error) {
      console.error("เสร็จสิ้น task error:", error);
      Alert.alert("Error", "Unable to update task status.");
    }
  };

  const handleยกเลิกเสร็จสิ้นTask = async (taskId) => {
    try {
      await undoTaskเสร็จสิ้น(taskId);
    } catch (error) {
      console.error("ยกเลิกเสร็จสิ้น task error:", error);
  ...
  const openRescheduleConflictModal = ({ result, newเริ่ม, newสิ้นสุด }) => {
  ...
  const saveRescheduledTask = async (task, newเริ่ม, newสิ้นสุด) => {
    if (!task?.id) {
      throw new Error("TASK_ID_REQUIRED");
    }

    const result = await rescheduleTask(task.id, newเริ่ม, newสิ้นสุด);
  ...
  const performQuickReschedule = async (targetType) => {
    ...
    try {
      ...
      const { newเริ่ม, newสิ้นสุด } = buildQuickRescheduledTimeRange(
  ...
  const performDeleteSingleTask = async () => {
    ...
    try {
      ...
      Alert.alert("Error", "ไม่สามารถลบงานนี้ได้");
    } finally {
      setIsDeleting(false);
      setDeleteModalVisible(false);
      setDeleteTargetTask(null);
  ...
  const performDeletePlanningWithSessions = async () => {
    ...
    try {
      ...
      Alert.alert("Error", "ไม่สามารถลบแผนและรอบอ่านทั้งหมดได้");
    } finally {
      setIsDeleting(false);
      setDeleteModalVisible(false);
      setDeleteTargetTask(null);
  ...
  const renderTaskCard = (task, sectionKey = "") => {
    const priority = getความสำคัญ(task);
    const priorityStyle = getความสำคัญStyle(priority);
    const accent = getTaskAccent(task);

    const isเสร็จแล้วLate = isเสร็จแล้วLateTask(task);
      ...
      <View key={task.id} style={styles.taskCard}>
        <Pressable
          style={[
            ...
            isเสร็จแล้วLate && styles.checkButtonLateActive,
          ]}
          onPress={() =>
            task.is_completed ? handleยกเลิกเสร็จสิ้นTask(task.id) : handleเสร็จสิ้นTask(task.id)
          }
        >
          {task.is_completed ? (
        ...
        <View style={styles.taskInfo}>
          <View style={styles.taskชื่องานRow}>
            <Text
              style={[
                styles.taskชื่องาน,
                task.is_completed && styles.taskชื่องานเสร็จแล้ว,
              ]}
              numberOfLines={1}
            >
          ...
          {task.detail ? (
            <Text style={styles.taskรายละเอียด} numberOfLines={1}>
              {task.detail}
            </Text>
          ) : null}
          ...
          <View style={styles.tagRow}>
            ...
            <View style={styles.repeatTag}>
              <Ionicons
                ...
                color={COLORS.primary}
              />
              <Text style={styles.repeatText}>{getการทำซ้ำLabel(task)}</Text>
            </View>
    ...
    return (
      <View key={section.key} style={styles.sectionBlock}>
        <View style={styles.sectionTop}>
          <View style={styles.sectionชื่องานBox}>
            <View
              style={[
                styles.sectionIcon,
                { backgroundColor: section.backgroundColor },
        ...
        {visibleงาน.map((task) => renderTaskCard(task, section.key))}
      </View>
    );
  };
    ...
    <View style={styles.container}>
      ...
      >
        <View style={styles.header}>
          <View>
            <Text style={styles.pageชื่องาน}>งาน</Text>
            <Text style={styles.pageSubtitle}>
              Organized by task type and study sessions
            </Text>
          </View>
        ...
        <Text style={styles.controlชื่องาน}>Filter</Text>
        ...
        <Text style={styles.controlชื่องาน}>Sort by</Text>
        ...
        <View style={styles.smartSuggestionCard}>
          <View style={styles.smartSuggestionTextBox}>
            ...
            <View style={styles.smartSuggestionCopy}>
              <Text style={styles.smartSuggestionชื่องาน}>Suggest Free Time</Text>
              <Text style={styles.smartSuggestionSubtitle}>
                Find available slots from your current schedule
              </Text>
            </View>
          </View>
        ...
        <View style={styles.listHeader}>
          <Text style={styles.listชื่องาน}>Task Sections</Text>
          <Text style={styles.listCount}>{filteredงาน.length} item(s)</Text>
        </View>

        {filteredงาน.length === 0 ? (
          <View style={styles.emptyCard}>
            <Ionicons
              ...
              color={COLORS.textMuted}
            />
            <Text style={styles.emptyชื่องาน}>No tasks found</Text>
        ...
        <Pressable style={styles.overlay} onPress={closeFreeTimeModal}>
          ...
          >
            <View style={styles.freeTimeModalHeader}>
              <View>
                <Text style={styles.modalชื่องาน}>Suggested Free Time</Text>
            ...
            {isLoadingFreeTime ? (
              ...
              <View style={styles.freeTimeEmptyBox}>
                <Ionicons
                  ...
                  color={COLORS.textMuted}
                />
                <Text style={styles.freeTimeEmptyชื่องาน}>
                  No free time found
                </Text>
              ...
              >
                {freeTimeSlots.map((slot, index) => {
                  const slotเริ่ม = normalizeDate(slot.start_time);
                  const slotสิ้นสุด = normalizeDate(slot.end_time);
                  const suggestedเริ่ม = normalizeDate(
                    slot.suggested_start_time
                  );
                  const suggestedสิ้นสุด = normalizeDate(slot.suggested_end_time);

                  return (
                    <View
                      key={`${slotเริ่ม?.toISOString() || "slot"}-${index}`}
                    ...
                    >
                      <View style={styles.freeTimeSlotTop}>
                        ...
                        <View style={styles.freeTimeSlotInfo}>
                          <Text style={styles.freeTimeSlotชื่องาน}>
                            {formatTaskTimeRange(suggestedเริ่ม, suggestedสิ้นสุด)}
                          </Text>
                          <Text style={styles.freeTimeSlotSubtitle}>
                            {formatTaskDateRange(suggestedเริ่ม, suggestedสิ้นสุด)} Â·{" "}
                      ...
                      <View style={styles.suggestedTimeBox}>
                        ...
                        <Text style={styles.suggestedTimeValue}>
                          {formatConflictDateTime(suggestedเริ่ม, suggestedสิ้นสุด)}
                        </Text>
                      </View>
      ...
      >
        <Pressable style={styles.overlay} onPress={closeRescheduleModal}>
          <Pressable
            ...
            onPress={(event) => event.stopPropagation()}
          >
            <Text style={styles.modalชื่องาน}>Reschedule Task</Text>
            ...
            {rescheduleTargetTask ? (
              <View style={styles.previewBox}>
                <Text style={styles.previewชื่องาน} numberOfLines={1}>
            ...
            >
              ...
              <Text style={styles.secondaryModalButtonText}>เลือกเวลาใหม่</Text>
            </Pressable>
      ...
      >
        ...
        >
          ...
          >
            <View style={styles.freeTimeModalHeader}>
              <View>
                <Text style={styles.modalชื่องาน}>Auto Find Best Time</Text>
                <Text style={styles.freeTimeModalSubtitle}>
            ...
            {isLoadingSmartReschedule ? (
              ...
              <View style={styles.freeTimeEmptyBox}>
                ...
                <Text style={styles.freeTimeEmptyText}>
                  {smartRescheduleMessage === "NO_SUITABLE_TIME_NEXT_7_DAYS"
                    ...
                    : "Try Move to Tomorrow or เลือกเวลาใหม่ instead."}
                </Text>
              </View>
            ) : (
              <ScrollView
              ...
              >
                {smartRescheduleSlots.map((slot, index) => {
                  ...
                  return (
                    <View
                      key={`smart-${slotเริ่ม?.toISOString() || "slot"}-${index}`}
                      style={styles.freeTimeSlotCard}
                    >
                      <View style={styles.freeTimeSlotTop}>
                        <View style={styles.freeTimeSlotIcon}>
      ...
      >
        <Pressable style={styles.overlay} onPress={closeRescheduleConflictModal}>
          ...
          >
            <View style={styles.freeTimeModalHeader}>
              <View>
                <Text style={styles.modalชื่องาน}>พบเวลาทับซ้อน</Text>
                <Text style={styles.freeTimeModalSubtitle}>
            ...
            {pendingRescheduleRange ? (
              <View style={styles.previewBox}>
                <Text style={styles.previewชื่องาน}>Requested New Time</Text>
                <Text style={styles.previewText}>
                  {formatConflictDateTime(
            ...
            >
              <Ionicons
                ...
                color={COLORS.textLight}
              />
              <Text style={styles.primaryModalButtonText}>เลือกเวลาใหม่</Text>
            </Pressable>
      ...
      >
        <Pressable style={styles.overlay} onPress={closeDeleteModal}>
          <Pressable
            ...
            onPress={(event) => event.stopPropagation()}
          >
            <Text style={styles.modalชื่องาน}>{getDeleteModalชื่องาน()}</Text>
              ...
              <>
                ...
                >
                  <Text style={styles.dangerModalButtonText}>
                    Delete แผนอ่านหนังสือ and All Sessions
                  </Text>
                </Pressable>
              </>
